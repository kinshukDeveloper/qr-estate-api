...
const { toEmbedUrl } = require('../services/tourService');
...
router.get('/:shortCode', asyncHandler(async (req, res) => {
  ...
  const listingWithEmbed = {
      ...listing,
      embed_url: listing.tour_url ? toEmbedUrl(listing.tour_url) : null,
    };
    res.json({ success: true, data: { listing: listingWithEmbed } });
}));

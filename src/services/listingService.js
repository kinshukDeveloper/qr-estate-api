// AI quality scoring — fire-and-forget on create/update
let aiService = null;
function getAI() {
  if (!aiService) {
    try { aiService = require('./aiService'); } catch {}
  }
  return aiService;
}
...
async function createListing(agentId, data) {
  ...
  // Fire-and-forget quality score computation
  const ai = getAI();
  if (ai) ai.computeQualityScore(created.id).catch(() => {});
  return created;
}
...
async function updateListing(listingId, agentId, data) {
  ...
  const ai = getAI();
  if (ai) {
    ai.computeQualityScore(listingId).catch(() => {});
    ai.invalidateCache(listingId).catch(() => {});
  }
  return updated;
}
